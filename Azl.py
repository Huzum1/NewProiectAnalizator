...
# --- NEW: FILTRU DISTANȚĂ ---
def check_min_distance(num_list, active=False):
    """
    Dacă activ=True, respinge variantele care au numere consecutive.
    Ex: 1, 2, 10 -> Respins (Distanța 1 < 2)
    """
    if not active: return True
    if len(num_list) < 2: return True
    
    # Sortăm pentru siguranță
    s_list = sorted(num_list)
    for i in range(len(s_list) - 1):
        if s_list[i+1] - s_list[i] < 2: # Distanța minimă 2 înseamnă că nu sunt consecutive
            return False
    return True

def check_quality_patterns(num_list):
    if len(num_list) < 3: return True
    # Păstrăm filtrul de triplete consecutive ca bază
...
def worker_analiza_hibrida(variante_brute, runde_config, top_n=100, evo_count=15, use_strict_filters=True, dist_filter=False):
    runde_engine = []
    ...
    rejected_pattern = 0 
    rejected_zombie = 0
    rejected_dist = 0 # Contor distanță
    ...
    for var in variante_brute:
        ...
        # 1. Filtru Distanță (Nou)
        if dist_filter and not check_min_distance(v_list, active=True):
            rejected_dist += 1
            continue
    ...
    diagnostics = {
        ...
        'dist': rejected_dist, # Raportăm și distanța
...
def main():
    ...
    with tab1:
                    ...
                    if uploaded_file is not None:
                        try:
                            ...
                            if parsed_imp:
                                ...
                                time.sleep(0.5) # Pauză scurtă pt UI
                                st.rerun() # REFRESH PENTRU A VEDEA DATELE
        ...
        # --- VERIFICARE DUPLICATE GLOBALĂ ---
        if all_rounds_flat:
            # Deduplicare
            initial_cnt = len(all_rounds_flat)
            unique_set = set(tuple(x) for x in all_rounds_flat)
            final_rounds = [list(x) for x in unique_set]
            removed = initial_cnt - len(final_rounds)
            
            # Folosim lista curată pentru analiză
            all_rounds_flat = final_rounds

            if removed > 0:
                st.warning(f"⚠️ Au fost detectate și eliminate {removed} runde duplicate! Analiza se face pe cele {len(final_rounds)} unice.")
            ...
            c3.metric("Runde Unice", len(all_rounds_flat))
    ...
    with tab2:
        ...
        with c1:
            ...
            # --- FILTRU DISTANȚĂ NOU ---
            use_dist = st.checkbox("🚫 Fără Numere Consecutive (Distanță)", value=False)
        ...
        with c2:
            ...
            if uploaded_vars:
                try:
                    ...
                    for l in content.split('\n'):
                        # Încercăm să parsam variante simple sau cu ID
                        parts = l.replace(',', ' ').split() # Tratează virgula ca spațiu
                        nums = [int(n) for n in parts if n.strip().isdigit()]
                        if len(nums) >= 2:
                             # Generăm un ID temporar dacă nu e clar
            ...
            if run and brute:
                with st.spinner("⚙️ Procesare..."):
                    res, n_evo, mb, dl, diag = worker_analiza_hibrida(
                        ...
                        dist_filter=use_dist # Trimitem setarea nouă
                    )
                ...
                if not res and (use_filters or use_dist):
                    ...
                    <div class="warning-box">
                        ...
                        <ul>
                            <li><b>Respinse Distanță (Consecutive):</b> {diag['dist']}</li>
